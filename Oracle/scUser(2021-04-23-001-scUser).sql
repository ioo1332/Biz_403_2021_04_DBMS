...
-- 영어 선생님 에게 전체 학생의 정보를 보여줘야한다
-- 다른과목점수는 감추고 싶다
-- 보안적인 측면에서 사용자별로
-- 보여줄항목,보이지 않을 항목을 선별하여 VIEW작성해두면 
-- 불필요한 정보가 노출되는것을 최소화할수있다
CREATE VIEW view_score
AS
(
...
AS
(
SELECT sc_num AS 학번,sc_eng AS 영어,sc_kor 국어,sc_math AS 수학
FROM tbl_score
WHERE sc_num>='s0010' AND sc_num<='SC0020'
);
